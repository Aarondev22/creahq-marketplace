...
export function useAuth(): AuthState {
  ...
  useEffect(() => {
    ...
    async function checkBanned(user: User | null) {
      if (!user) return false;
      const { data } = await supabase.from("profiles").select("banned").eq("id", user.id).maybeSingle();
      return data?.banned ?? false;
    }

    function apply(session: Session | null) {
      const user = session?.user ?? null;
      Promise.all([loadRoles(user), checkBanned(user)]).then(async ([roles, banned]) => {
        if (!mounted) return;
        if (banned) {
          await supabase.auth.signOut();
          setState({ user: null, session: null, loading: false, roles: [], isFounder: false, isAdmin: false, isSeller: false });
          return;
        }
        setState({
          user,
