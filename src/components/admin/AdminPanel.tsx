...
import { X, Minus, GripHorizontal, Users, Store, Tag, Star, Megaphone, BarChart3, Search, Ban, ShieldCheck, ArrowUp, ArrowDown, Trash2 } from "lucide-react";
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-3">
      ...
      <div className="rounded-xl border border-dashed border-brand/40 bg-brand-soft/30 p-4 text-xs text-muted-foreground sm:col-span-3">
        Live-GMV und Streitfälle-Übersicht folgen mit Stripe (Welle 2).
      </div>
    </div>
  );
}
...
function ShopsTab() {
  ...
  async function removeAllListings(shop: ShopRow) {
    if (!confirm(`Wirklich ALLE ${shop.listingCount} Listings von ${shop.display_name ?? "diesem Shop"} löschen?`)) return;
    const { error } = await supabase.from("listings").delete().eq("seller_id", shop.seller_id);
    if (error) return toast.error(error.message);
    toast.success("Listings entfernt");
    load();
  }
  ...
  return (
    <ul className="space-y-2">
      {shops.map((s) => (
        <li key={s.seller_id} className="flex items-center justify-between gap-2 rounded-xl border border-border bg-surface p-3">
          <div className="min-w-0">
          ...
          <div className="flex shrink-0 gap-1.5">
            <button
              ...
              className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-bold ${s.featured ? "bg-amber-100 text-amber-800" : "bg-brand-soft text-brand-ink"}`}
            >
              <Star className="h-3 w-3" /> {s.featured ? "Featured" : "Featuren"}
            </button>
            <button onClick={() => removeAllListings(s)} className="inline-flex items-center gap-1 rounded-full bg-red-50 px-2.5 py-1 text-[11px] font-bold text-red-600">
