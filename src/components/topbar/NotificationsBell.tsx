      ...
      <PopoverContent align="end" className="w-[22rem] overflow-hidden rounded-3xl p-0">
        <div className="flex items-center justify-between gap-2 border-b border-border bg-brand-soft/40 px-4 py-3">
          <div>
            ...
            <div className="text-xs text-muted-foreground">{unread > 0 ? `${unread} ungelesen` : "Alles gelesen 🎉"}</div>
          </div>
          {unread > 0 && (
            <button
              onClick={markAllRead}
        ...
        {!signedIn ? (
          <div className="px-4 py-10 text-center text-sm text-muted-foreground">Melde dich an, um Benachrichtigungen zu sehen.</div>
        ) : loading ? (
          ...
          <div className="px-4 py-10 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">Nachrichten, Bestellungen und Versand landen hier.</p>
          </div>
        ) : (
          <ul className="max-h-96 divide-y divide-border overflow-y-auto">
            {items.map((n) => (
              <li key={n.id}>
                <button
                  onClick={() => open(n)}
                  className={`flex w-full items-start gap-3 px-4 py-3 text-left transition-colors hover:bg-brand-soft/50 ${n.read_at ? "" : "bg-brand-soft/25"}`}
                >
